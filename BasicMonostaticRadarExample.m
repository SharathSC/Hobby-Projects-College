...
% This example shows how to design a monostatic pulse radar to estimate the
% target range. A monostatic radar has the transmitter colocated with
% the receiver. The transmitter generates a pulse which hits the target and
% produces an echo received by the receiver.  By measuring the location of
% the echoes in time, we can estimate the range of a target.
%
% This example focuses on a pulse
% <http://www.mathworks.com/discovery/radar-system-design.html radar system
% design> which can achieve a set of design specifications.  It outlines
% the steps to translate design specifications, such as the probability of
% detection and the range resolution, into radar system parameters, such as
% the transmit power and the pulse width. It also models the environment
% and targets to synthesize the received signal. Finally, signal processing
% techniques are applied to the received signal to detect the ranges of the
% targets.
...
% <ChirpRangeEstimationExample.html Waveform Design to Improve Performance
% of an Existing Radar System> for an example using a chirp waveform.
